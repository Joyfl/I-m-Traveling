...
//	[[SettingsManager manager] clearSettingForKey:SETTING_KEY_LOCAL_SAVED_TRIPS]; [[SettingsManager manager] clearSettingForKey:SETTING_KEY_LOCAL_SAVED_FEEDS]; [[SettingsManager manager] flush];
	...
	DLog( @"로컬에 저장된 여행 개수 : %d개", _trips.count );
	...
	DLog( @"로컬에 저장된 피드 개수 : %d개", _feeds.count );
...
- (NSInteger)addTrip:(NSMutableDictionary *)trip
{
	...
	DLog( @"새로운 여행(localTripId=%d) 추가 후 여행 개수 : %d", localTripId, _trips.count );
...
- (void)addFeed:(NSMutableDictionary *)feed
{
	...
	DLog( @"새로운 피드 추가 후 피드 개수 : %d", _feeds.count );
...
- (void)uploadTrip:(NSMutableDictionary *)trip
{
	...
	DLog( @"업로드 할 여행 (localTripId=%d) : %@", localTripId, trip );
...
{
	DLog( @"업로드 결과(id=%d) : %@", token.tokenId, token.data );
	...
	if( token.tokenId < 0 )
	{
		...
		for( NSInteger i = 0; i < _feedUploader.queueLength; i++ )
		{
			...
			DLog( @"_feeds 루프돌리는 중, trip_id : %d", [[params objectForKey:@"trip_id"] integerValue] );
			
			if( [[params objectForKey:@"trip_id"] integerValue] == localTripId )
			{
				DLog( @"trip id가 %d인 피드 발견!! %d로 바꿈!", localTripId, tripId );
		...
		DLog( @"업로드가 완료된 여행 제거 후 여행 개수 : %d", _trips.count );
	...
	else
	{
		...
		DLog( @"업로드가 완료된 피드 제거 후 피드 개수 : %d", _feeds.count );
